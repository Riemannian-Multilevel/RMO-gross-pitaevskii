...
void make_grid(Triangulation<2> &triangulation, const int levels = 3)
{
...
// freedom</i>).
void distribute_dofs(DoFHandler<2> &dof_handler, const int degree = 1)
{
  const FE_Q<2> finite_element(degree);
...
int main(int argc, char *argv[])
{
    ...
    try {
        ...
        polynomial_degree = vm["degree"].as<int>();
    ...
    make_grid(triangulation, levels);

    DoFHandler<2> dof_handler(triangulation);
    distribute_dofs(dof_handler, polynomial_degree);
