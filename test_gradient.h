...
template <int dim>
class GradientTestCoarseMassEnergyAdaptive : public GradientTestCoarse<dim>
{
public:
    using GradientTestCoarse<dim>::GradientTestCoarse;
    ...
    {
        Vector<double> q_grad(x.size());
        coarse::mass::energy_adaptive_gradient(this->M, this->A_inv, x, this->m_phi, this->m_w, q_grad);
...
template <int dim>
class GradientTestCoarseFrobeniusEnergyAdaptive : public GradientTestCoarse<dim>
{
public:
    using GradientTestCoarse<dim>::GradientTestCoarse;
    ...
    double value(const Vector<double>& x) const override
    {
        ...
        return coarse::frobenius::function_value(x, this->m_phi, this->m_w, M, A0, Mpp, this->m_beta);
    }
    ...
    void random_tangent_vector(const Vector<double>& x, Vector<double>& v) const final
    {
        Vector<double> tmp(v.size());
        normrnd(this->mean, this->stddev, tmp);
        ellipsoid::energy::project_onto_tangent_space(this->A_inv, x, this->M, tmp, v);
    }
    ...
    void to_tangent_space(const Vector<double>& x, const Vector<double>& v, Vector<double>& v_proj) const final
    {
        ellipsoid::energy::project_onto_tangent_space(x, this->M, v, v_proj);
    }
};
