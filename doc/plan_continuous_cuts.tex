...
Every module in Table~\ref{tab:status} is implemented and tested, including a two-level
driver wiring the coarse model into the framework's generic multilevel solver
(\texttt{FullApproximationScheme}). It runs correctly and stably, but does not demonstrate
a clear speedup over the single-level baseline on the synthetic problem it uses -- getting
one would need per-problem tuning beyond this module's scope; see \S\ref{sec:mlcc-design}.
...
exactly as in eq.~(41)/(42), with $(\alpha,\varepsilon)=(0.1,10^{-4})$ matching the paper's
fine-level parameters (\S 6.3.4). It wires \texttt{FisherRaoOracle} and
\texttt{BernoulliManifold} directly into the already problem-independent
\texttt{GradientDescent} solver and \texttt{ConvergenceTableObserver} -- no CC-specific
solver code was needed. On a $41\times41$ grid it converges in 147 iterations (residual
$10^{-6}$, energy from $182.7$ to $-112.2$, monotonically), with a reduction rate that
drifts between $1.08$ and $1.26$ rather than settling at a fixed ratio -- i.e.\ visibly
...
\begin{itemize}[nosep]
  ...
  \item \textbf{The coarse model can run away to the boundary.} $q_k(z)$'s correction term
        ...
        not a bug to fix in \texttt{oracle\_coarse.h} -- the coarse solver's initial step
        size is simply damped (\texttt{ls.alpha}$=0.01$) so backtracking gets a chance to
        engage before a single step can do this.
\end{itemize}
...
should. What it does \emph{not} do is demonstrate a speedup over that single-level baseline:
the ratio $\|R\operatorname{grad}f_h(x)\|/\|\operatorname{grad}f_h(x)\|$ the trigger
condition (eq.~(16)) compares against $\eta$ ranges from about 2 early in the run to over
200 later, rather than staying near one scale, so a single constant $\eta$ either triggers a
correction on virtually every iteration (a low $\eta$, as with the paper's own $\eta=0.6$ for
this problem -- each correction mildly useful but not free, and slower overall on this
synthetic image than the single-level baseline) or never triggers at all (any $\eta$ above
about 2, which just silently disables the coarse model and reproduces the single-level run).
A second, unrelated deviation from the reference was tested and ruled out as a contributing
cause. \texttt{main\_cc\_coarse.cc} originally built its coarse image by injecting the fine
one and recomputing $\rho$ at coarse resolution; \texttt{problem.py} instead computes $\rho$
once at the finest resolution and average-pools \emph{that} down to each coarser level (the
seed patches are applied exactly once). The driver now matches the reference -- $\rho$ is
coarsened directly, via full-weighting ($F_h^H=\tfrac14\texttt{Tfine}$, this grid's analogue
of average-pooling, since a coarse pixel here coincides with a fine one rather than sitting
at a $2{\times}2$ block's center) -- but the outcome is essentially unchanged: final energy
after 300 iterations is $-94.01$ (was $-93.99$), and $\|\operatorname{grad}f_h\|\approx2.5$
against $\|R\operatorname{grad}f_h\|\approx548$ at the end, the same $\sim\!220\times$ gap as
before. So how the coarse data term is built was not the cause of the poor convergence; the
scale mismatch above is squarely a property of Option 1's vector transport
($R=4F_h^H$), independent of it.

\paragraph{Two further causes, found by reading \texttt{operators.py}/\texttt{multilevel.py}
directly.} First, a genuine gap: the reference's trigger check, \texttt{c\_check}, does not
compare $\|R\operatorname{grad}f_h\|$ against $\eta\|\operatorname{grad}f_h\|$ directly -- it
scales the coarse side by a per-approach \texttt{grid\_scale} first
(\texttt{scaled\_norm\_coarse = grid\_scale * norm\_coarse}), and for Option 1 this factor is
$2^{n_{\text{pools}}}$ ($=2$ for one factor-2 step). \texttt{FullApproximationScheme}'s own
check (\texttt{fas.h}, \texttt{norm\_coarse >= options\_fas.kappa * norm\_level}) has no such
factor, so the $\eta=0.6$ used here and the paper's $\eta=0.6$ (Fig.~15) are not actually
comparable: reproducing the reference's trigger exactly would need an extra
\texttt{grid\_scale} multiplier on \texttt{norm\_coarse} before the comparison. This gap is now
fixed, generally rather than just for this one factor (\S\ref{sec:condition-design}), but by
itself the factor only rescales the comparison by a constant, and cannot explain a ratio that
itself grows from about 2 to about 220 over the course of the run --- confirmed by running
\texttt{main\_cc\_coarse.cc} with the \texttt{grid\_scale}$=2$ condition in place: the trigger
still fires on every iteration and the final energy is unchanged to five digits.

Second, and more load-bearing: that growth is consistent with $R$'s own character. $R=4F_h^H$
is a \emph{summing} restriction (its stencil coefficients are all positive and sum to 4 -- the
same DC-gain-4 stencil as the reference's \texttt{restrict\_adjoint\_bilinear}, verified
identical coefficient-for-coefficient), not an averaging one; whether it grows or shrinks a
given gradient's norm depends on how spatially coherent that gradient is. Early in the descent,
$\operatorname{grad}f_h$ is dominated by the smooth, diffuse pull of the data term $\rho$
(quadratic difference-of-means, varying gently over the image) -- $R$'s summing barely changes
its norm, so the ratio sits near the DC-gain-driven value of about 2--4. As the segmentation
sharpens, the TV term comes to dominate $\operatorname{grad}f_h$ and concentrates into a narrow
band along the emerging interface, where neighboring fine pixels' gradient values are large and
of the same sign over several pixels at once (the interface's width, set by the
$\varepsilon$-smoothing) -- exactly the locally-coherent regime a summing operator amplifies
rather than attenuates. So the $\sim\!220\times$ gap by the end of the run is not a fixed
discretization artifact but tracks how ``DC-like'' the fine gradient has become near
convergence; it compounds with, but is distinct from, the missing \texttt{grid\_scale} factor
above. (The forward-difference operator's lack of a $1/h$ mesh-spacing normalization was also
considered and ruled out as a cause: \texttt{objective.py}'s \texttt{get\_isotropic\_tv} has no
such factor either, so \texttt{ForwardDifference} matches the reference exactly here.)
