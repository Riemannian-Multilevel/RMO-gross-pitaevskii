...
matching the Python reference on the same problem. The port still cannot run the paper's own
$960\times1280$ experiment (\S6.3): the grid transfer requires \texttt{fine = 2\textperiodcentered
coarse}$-1$ per dimension and has no operator composition for the paper's two pooling steps,
no image input, and none of Options 2/3/5 -- that is the module's actual scope gap, tracked as
future work, not a tuning problem on the synthetic image.
