...
#suffix <- ".org"
#all_ml_files <- list.files(pattern = paste0("ml_.*_l11.*", suffix))
all_ml_files <- list.files(pattern = paste0("^ml_.*_l11.*", gsub(".", "\\.", suffix, fixed = TRUE), "$"))
prefixes     <- unique(str_remove(all_ml_files, paste0("_depth[2-6]", suffix)))

for (pref in prefixes) {
  b_key      <- str_extract(pref, "b[0-9]+")
  ref_energy <- ref_energies[b_key]
  
  d2   <- load_depth(paste0(pref, paste0("_depth2", suffix)), "Depth2", TRUE, ref_energy)
  d3   <- load_depth(paste0(pref, paste0("_depth3", suffix)), "Depth3", TRUE, ref_energy)
  d4   <- load_depth(paste0(pref, paste0("_depth4", suffix)), "Depth4", TRUE, ref_energy)
  #d5   <- load_depth(paste0(pref, paste0("_depth5", suffix)), "Depth5", TRUE, ref_energy)
  #d6   <- load_depth(paste0(pref, paste0("_depth6", suffix)), "Depth6", TRUE, ref_energy)
  d_sl <- load_depth(paste0("sl_", b_key, paste0("_l11", suffix)), "SL", FALSE, ref_energy)
  ...
  time_df  <- plot_df[plot_df$elapsed <= 200, ]
